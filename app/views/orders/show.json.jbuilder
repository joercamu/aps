...
json.subprocesses @order.subprocesses.order(:sequence_process) do |subprocess|
	json.extract! subprocess, :id, :order_id, :procedure_id, :standard_id, :minutes, :start_date, :end_date, :meter, :sequence, :state, :created_at, :updated_at,:setup_time,:day_id,:sequence_process
	json.day_date subprocess.day.day if subprocess.day
	...
	json.days subprocess.standard.machine.days.availables.where('available >= ?',subprocess.minutes) do |day|
		...
		json.last_position day.subprocesses.count+1
	end
end
